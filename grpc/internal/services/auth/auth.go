...
// Login checks if user giving credentials exists in the system.
//
...
func (a *Auth) Login(
	...
	if err != nil {
		a.log.Error("failder ro generate token", slog.String("error", err.Error()))
		return "", fmt.Errorf("%s: %w", op, err)
	}
	return token, nil
}
...
func (a *Auth) RegisterNewUser(
	...
	if err != nil {
		log.Error("failed to generate has from pass", slog.String("error", err.Error()))
