...
import { createService, updateNodePosition } from '../functions/api/api';
...
    const currentRepoUrl = localStorage.getItem('repo'); // Получаем полный URL репозитория
...
        fetch(`https://mixail.ermin33.fvds.ru/api/repo-tree?repo=${encodeURIComponent(currentRepoUrl)}`)
            .then(async res => {
...
                // Используем fetchedStructure.name как имя проекта, оно придет из бэкенда
...
        // Для updateNodePosition также нужен Project Name
        // Используем structure.name, если доступно, иначе currentRepoUrl.split('/').pop()
...
            }
        }
    }, [structure, currentRepoUrl]); // Добавляем structure и currentRepoUrl в зависимости
...
        // Используем structure.name - это имя проекта, которое бэкенд ожидает
...
        try {
            // ИЗМЕНЕНИЕ: Используем structure.name вместо currentRepoUrl
...
            const newNode = createReactFlowServiceNode(result.serviceId, serviceType, position, result.name); // Передаем result.name если бэкенд его возвращает
...
        }
    }, [structure, contextMenu, setNodes]); // Добавляем structure в зависимости
...
                                {/* ИЗМЕНЕНИЕ: Отображаем structure.name для текущего репо */}
